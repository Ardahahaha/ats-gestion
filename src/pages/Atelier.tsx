...
import { Plus, Trash2, Table2, Columns3, ChevronRight, ArrowLeft } from "lucide-react";
...
function TableView({
  ...
  const fetchAll = useCallback(async () => {
    const [colRes, rowRes, cellRes] = await Promise.all([
      ...
      supabase
        ...
        .in(
          "row_id",
          (await supabase.from("custom_rows").select("id").eq("table_id", table.id)).data?.map((r) => r.id) ?? []
        ),
    ]);
  ...
  const updateCell = async (rowId: string, colId: string, value: string) => {
    ...
    const { error } = await supabase.from("custom_cells").upsert(
      { row_id: rowId, column_id: colId, value },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={onBack}
          className="flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-secondary text-muted-foreground transition-all duration-200 hover:border-amber-500/50 hover:text-amber-500"
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          onClick={addRow}
          size="sm"
          className="gap-1 bg-amber-500 font-display text-xs font-semibold uppercase tracking-wider shadow-lg shadow-amber-500/25 hover:bg-amber-600"
        >
      ...
      {columns.length === 0 ? (
        <div className="flex flex-col items-center gap-3 py-16 rounded-xl border border-border bg-card">
          ...
          <p className="font-display text-sm uppercase tracking-widest text-muted-foreground">
            Ajoutez des colonnes pour commencer
          </p>
        </div>
      ) : (
              ...
              <tr className="bg-table-header text-table-header-foreground">
                {columns.map((col) => (
                  <th key={col.id} className="border-r border-white/10 px-4 py-4 text-left last:border-r-0">
                    <div className="flex items-center gap-2">
                      <span className="font-display text-xs font-semibold uppercase tracking-widest truncate">
                        {col.name}
                      </span>
                      <button
                        onClick={() => deleteColumn(col.id)}
                        className="ml-auto shrink-0 rounded p-1 text-white/40 hover:bg-white/10 hover:text-destructive transition-colors"
              ...
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={columns.length + 1} className="py-12 text-center">
                    <p className="font-display text-sm uppercase tracking-widest text-muted-foreground">
                      Aucune ligne — cliquez sur "+ Ligne"
                    </p>
                  </td>
                </tr>
              ) : (
...
const Atelier = () => {
  ...
  if (selected) {
    return <TableView table={selected} onBack={() => { setSelected(null); fetchTables(); }} />;
  }

  return (
    <TableList tables={tables} onSelect={setSelected} onCreate={createTable} onDelete={deleteTable} />
  );
};
